...
export class InflowsService {
  ...
  constructor(
    ...
    private readonly inflowsRepository: Repository<Inflow>,
    @InjectRepository(InflowType)
    private readonly inflowsTypeRepository: Repository<InflowType>,
    @InjectRepository(Deposit)
    private readonly depositRepository: Repository<Deposit>,
    @InjectRepository(InflowDeposit)
    private readonly inflowDepositRepository: Repository<InflowDeposit>,
  ...
  async findAll(userId, page: IPagination) {
    ...
    const inflows:Pagination<any, IPaginationMeta> = await this.paginationService.paginate(
      this.inflowsRepository,
      page,
      opts,
    );
    const inflowsMapped = inflows.items.map(this.handleInflow.bind(this));
    const newItems= await Promise.all(inflowsMapped);
    return {
      ...inflows,
      items: newItems
    }
    ...
    try {
      ...
      for (let porcent of porcents) {
        const { id } = await queryRunner.manager.save(
          await this.prepareInflowDeposit(porcent, inflowSaved, user),
        );
  ...
  async update(userId: number, inflowId: number, inflowIn: UpdateInflowDto) {
    ...
    await queryRunner.startTransaction();
    try {
      const inflow = await this.inflowsRepository.findOne({
        where: { id: inflowId, user },
      });
      if (!inflow) {
        throw new BadRequestException(
          `This inflow by id ${inflowId} doesn't exists`,
        );
      }
      inflow.inflowtype = inflowType;
      inflow.description = description;
      ...
      for (let porcent of porcents) {
        await queryRunner.manager.save(
          await this.prepareInflowDepositUpdate(
            porcent,
            inflowSaved,
            porcent.inflowDepositId,
            user,
          ),
        );
      }
  ...
  private async prepareInflowDepositUpdate(
    porcent: IInflowPorcent,
    inflow: Inflow,
    porcentId?: number,
    user?: User,
  ) {
    const deposit: Deposit = await this.depositRepository.findOne({
      where: { id: porcent.depositId, user },
    });
    if (!deposit) {
      throw new BadRequestException('The deposit not exists');
    }
    const inflowDeposit: InflowDeposit =
      await this.inflowDepositRepository.findOne({
        where: { id: porcentId, deposit, inflow },
      });
    if (!inflowDeposit) {
  ...
  private async getDeposits(inflowdeposits) {
    const handleGetDeposits = async ({ id, ...rest }) => {
      ...
      return {
        deposit:
          (await this.depositRepository.findOne(inflowDeposit.deposit.id)) ||
          {},
  ...
  private async prepareInflowDeposit(
    porcent: IInflowPorcent,
    inflow: Inflow,
    user: User,
  ) {
    const deposit: Deposit = await this.depositRepository.findOne({
      id: porcent.depositId,
      user,
    });
    if (!deposit) {
      throw new BadRequestException('The deposit not exists');
  ...
  private validateSumPercents(porcents: IInflowPorcent[]) {
    porcents.forEach(({ porcent, depositId }) => {
      if (!porcent || !depositId) {
        throw new BadRequestException(
          'Please send in a correct way the porcent and the depositId',
        );
      }
    });
    const sumPorcents = porcents
      .map(({ porcent }) => porcent)
      .reduce((prev, curr) => prev + curr, 0);
    if (sumPorcents !== 100) {
      throw new BadRequestException(
        'The sum of the porcent must be equal to 100',
      );
    }
  }
}
